...
public:
    ...
    void enqueue()
    {
        ...
        queueLength++;
        cout << "Queue1 Length: " << queueLength << endl;
...
public:
    ...
    void enqueue()
    {
        Customer *temp = new Customer();
        queueLength++;
        cout << "Queue2 Length: " << queueLength << endl;
        cout << "New customer at " << storeClock / 60 << " hours and wait time " << temp->waitTime << " minutes " << endl;
        serviceWaitTime = temp->waitTime;
        wait_time_sum += temp->waitTime;
    ...
    while (storeClock < 1020)
    {
        ...
        if (storeClock <= 120)
        {
            ...
            if (random_number <= 30)
            {
                ...
                customerCount++;
                if (myStore.queueLength == 0 && myStore2.queueLength == 0)
                {
                    myStore.enqueue();
                }
                else if (myStore2.queueLength < myStore.queueLength)
                {
                    myStore2.enqueue();
                }
                else
...
//No new customer
        ...
        if (myStore2.head != NULL)
        {
            if (myStore2.head->waitTime == 1)
            {
                cout << "A customer has left the line ";
                myStore2.head->orderTime = rand() % 6 + 1;
                cout << "with order prep time " << myStore2.head->orderTime << " minutes" << endl;
                myStore2.service_time_sum += myStore2.head->orderTime;
                storeClock += myStore2.head->orderTime;
                if (myStore2.min_service_time > myStore2.head->orderTime + myStore2.serviceWaitTime)
                {
                    myStore2.min_service_time = myStore2.head->orderTime + myStore2.serviceWaitTime;
                    min_service_clock = storeClock;
                }
                else if (myStore2.max_service_time < myStore2.head->orderTime + myStore2.serviceWaitTime)
                {
                    myStore2.max_service_time = myStore2.head->orderTime + myStore2.serviceWaitTime;
                    max_service_clock = storeClock;
                }
                myStore2.dequeue();
            }
